...
plot1 <- ggplot(x, aes(x = numOfIters, y = accuracy)) + geom_line(lwd = 1.5) + labs(x = "Number of Iterations", y = "Accuracy (%)", title = "Accuracy vs number of iterations")

plot2 <- ggplot(x, aes(x = numOfIters, y = timeModel)) + geom_line(lwd = 1.5) + labs(x = "Number of Iterations", y = "Speed (seconds)", title = "Time vs number of iterations")
...
ggplot(x, aes(x = acc, y = algoName)) + geom_bar(stat = 'identity', fill = c("darkred", "steelblue1", "steelblue1", "springgreen3")) + 
  ...
  labs(x = "Accuracy (%)", y = "Name of algorithm", title = "Performance selected algorithms") +
  annotate(x = 75, y = 4, geom = "text", label = " <--- Model build time")
```
Their is seen that Bagging preforms the overall best, it is to 'slowest' of the 4 but still fast enough for not heavy usage. If we recall the quality metrics: The demand was a high scoring algorithm that also was fast. A good confusion matrix was also desirable. 
