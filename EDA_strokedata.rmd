...
```
As is shown, most people have been married at some point in their lives. The not-married group is a sizeable group with people that have not been married. This maybe could been young people, which would be logic. The code below tests this hypothesis:
...
Let's take a look a living area. The living area's are rural or urban. Earlier is said that the mean age of living is not that different. So, the assumption that young people, that mostly life in urban regions, decreases the percentage of strokes events in that living region can be rejected. 
...
In the distribution above is clearly seen that strokes mostly affects older people, which is expected. The mean age of stroke is 67 years old. Also it is more frequent with female than with male. 
...
```{r}
x <- SMOTE(form = stroke ~ ., data = health, perc.over = 10, perc.under = 1)
```
